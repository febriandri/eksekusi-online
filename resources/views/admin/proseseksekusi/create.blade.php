...
<a href="{{ url()->previous() }}" class="btn btn-sm btn-primary" role="button"><i class="fa-solid fa-rotate-left"></i> Kembali</a>
    ...
    <div class="col-md-12">
        <div class="main-card mb-3 card">
            <div class="card-body">
                ...
                <div class="col-lg-8">
                    <form action="/admin/putusan-inkrah/{{ $inkrah->slug }}" method="post">
                        @csrf
                        @method('put')
                        <div class="mb-3">
                            <label for="alur_detail_id" class="form-label">Tahapan</label>
                            <select class="form-select" name="alur_detail_id">
                                <option selected>Pilih</option>
                                @foreach ($tahapans as $tahapan)
                                    <option value="{{ $tahapan->id }}">{{ $tahapan->tahapan->nama }}</option>
                                @endforeach                                
                            </select>
                        </div>
